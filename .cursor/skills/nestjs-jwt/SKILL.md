...
- Env: `JWT_SECRET` (required), `JWT_EXPIRES_IN`, `JWT_REFRESH_SECRET` (optional fallback to `JWT_SECRET`), `JWT_REFRESH_EXPIRES_IN` (default `7d`), `COOKIE_SECURE` (`true` / `false` overrides `NODE_ENV`; unset → secure in production), `REFRESH_COOKIE_PATH`, `CORS_ORIGIN` (with `credentials: true`), `BOOTSTRAP_ADMIN_*`
- Cookie helpers: `apps/api/src/auth/refresh-cookie.ts` (`maghami-system_refresh`, HttpOnly, SameSite=Lax)
